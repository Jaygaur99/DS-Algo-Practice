...
/*------------------------------------------------------*/
/*
State transition all in onenote
*/
...
int subSetSumRepeatingNumber(int index, int sum){
    if(index == -1){
    ...
    // include a[index]
    if(sum >= a[index])
        ans |= subSetSumRepeatingNumber(index, sum-a[index]);
    // exclude a[index]
    ans |= subSetSumRepeatingNumber(index-1, sum);
...
void solve(){
    int sum;
    cin>>sum>>n;
    memo.resize(n, vi(sum+1, -1));
    rep(i, 0, n){
        cin>>a[i];
    }

    cout<<subSetSumRepeatingNumber(n-1, sum);
}
